...
public class AnswerComparator implements Comparator<String> {
	...
	/** Index of first accented char. **/
	private static final int MIN = 192;
	/** Index of last accented char. **/
	private static final int MAX = 255;
	
	/** Matching map between accented / non accented chars. **/
	private static final List<String> map = initMap();
	
	private Locale locale;
	
	private AnswerComparator(Locale locale) {
		this.locale = locale;
	}
	...
	public int compare(String currentWordValue, String message) {
	    ...
	    // First comparison
		int result = compareOperator.compare(currentWordValue, message);      
		
		// If not matching try manual comparison
		if(result != 0) {
			String convertedCurrentWordValue = convert(currentWordValue);
			String convertedMessage = convert(message);
			
			boolean isAnswer = convertedCurrentWordValue.equals(convertedMessage);
			
			// If still not matching check message does not contain answer
			if(!isAnswer) {
				isAnswer = convertedMessage.contains(convertedCurrentWordValue);
			}
			
			// If still not matching, answer not found
			if(!isAnswer) {
				return -1;
			}
		}
	...
	{ 
		...
		{ int carVal = string.charAt(bcl);
		if( carVal >= MIN && carVal <= MAX )
		{ // Replacement
			java.lang.String newVal = (String)map.get( carVal - MIN );
    ...
    static Map<Locale, AnswerComparator> instances = new HashMap<Locale, AnswerComparator>();
    public static AnswerComparator getInstance(Locale locale) {
    	AnswerComparator instance;
        
    	if(instances.containsKey(locale)) {
    		instance = instances.get(locale);
        } else {
        	instance = new AnswerComparator(locale);
